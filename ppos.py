...
# --- 사용자 설정 ---
RENDER_MODE = False    # [수정됨] True: 화면 표시 (느림), False: 화면 끔 (빠름)
TOTAL_EPISODES = 5000  # 총 에피소드 수
MAX_STEPS = 400        # 에피소드 당 최대 스텝
MAZE_OPENNESS = 0.6    # 미로 개방도 (낮을수록 벽이 많음)
# ------------------

print(f"Device: {device}")
print(f"Render Mode: {'ON' if RENDER_MODE else 'OFF'}")
print(f"Total Episodes per Agent: {TOTAL_EPISODES}")
print(f"Maze Openness: {MAZE_OPENNESS}")

# ==============================================================================
# 1. Unified Maze Environment
# ==============================================================================
class MazeEnv:
    def __init__(self, maze_width=8, maze_height=8, cell_size=1.0, render_mode=False, max_steps=400):
        ...
        if self.render_mode:
            ...
            self.pygame.display.set_caption("RL Training")
        self.reset()
...
class PPOAgent:
    def __init__(self, state_dim, action_dim, lr=0.0003):
        ...
        self.action_var = torch.full((action_dim,), 0.36).to(device) # std=0.6
...
class RPPOAgent:
    ...
    def update(self):
        ...
        # Pre-processing for Sequence
        states = torch.cat(self.buffer['s'], dim=1).detach() # (1, N, dim)
        actions = torch.cat(self.buffer['a'], dim=0).detach() # (N, dim)
        logprobs = torch.cat(self.buffer['lp'], dim=0).detach()
        hiddens = torch.cat(self.buffer['h'], dim=1).detach() # (1, N, dim)

        N = len(rewards)
        n_seq = N // self.seq_len
        if n_seq == 0: return # 데이터 부족시 스킵
        ...
        b_h = hiddens[:, 0:n_seq*self.seq_len:self.seq_len, :]

        for _ in range(self.K_epochs):
            a_mean, val, _ = self.policy(b_states, b_h)
            cov = torch.diag(self.policy.action_var).unsqueeze(0).expand_as(a_mean)
...
class HPPOAgent(PPOAgent):
    def __init__(self, state_dim, action_dim, stack=3):
        super().__init__(state_dim * stack, action_dim) # Input dim 증가
        self.stack = stack

# ==============================================================================
# 5. CPPO Agent Class (Optimized Logic from main.py, Fixed)
# ==============================================================================
class CPPOAgent:
    def __init__(self, state_dim, action_dim):
        # Parameters similar to main.py
        self.lr = 1e-4
        self.gamma = 0.99
        self.eps_clip = 0.15
        self.K_epochs = 5
        self.batch_size = 512
        self.stack_frames = 4
        self.input_dim = state_dim * self.stack_frames
        self.hidden_size = 512
        
        self.policy = nn.Sequential(
            nn.Linear(self.input_dim, self.hidden_size), nn.ReLU(),
        ...
        self.action_var = nn.Parameter(torch.zeros(action_dim)).to(device)
        self.optimizer = optim.Adam(list(self.policy.parameters()) + list(self.critic.parameters()) + [self.action_var], lr=self.lr)
    ...
    def select_action(self, state):
        with torch.no_grad():
            ...
            a_mean = self.old_policy(s)
            cov = torch.diag(torch.exp(self.action_var)**2).unsqueeze(0)
            dist = torch.distributions.MultivariateNormal(a_mean, cov)
            action = dist.sample()
            lp = dist.log_prob(action)
            val = self.critic(s)
    ...
    def update(self):
        rewards = []
        vals = torch.cat(self.buffer['v']).squeeze().detach().cpu().numpy()
        # GAE approximation (simplified for batch)
        gae = 0
        ...
        for _ in range(self.K_epochs):
            ...
            for start in range(0, dataset_size, self.batch_size):
                ...
                a_mean = self.policy(b_s)
                cov = torch.diag(torch.exp(self.action_var)**2).unsqueeze(0).expand_as(a_mean)
                dist = torch.distributions.MultivariateNormal(a_mean, cov)
                
                lp = dist.log_prob(b_a)
                ent = dist.entropy().mean()
                val = self.critic(b_s).squeeze()
...
# ==============================================================================
# 6. Training Functions per Algorithm
# ==============================================================================
def train_ppo():
    print("\n>>> [1/4] Starting PPO Training...")
    env = MazeEnv(render_mode=RENDER_MODE, max_steps=MAX_STEPS)
    ...
    for ep in range(TOTAL_EPISODES):
        ...
        steps = 0
        while not done:
            if RENDER_MODE: env.render() # [추가] 렌더링 확인
            ...
            score += reward
            steps += 1
            if steps % 2048 == 0: agent.update()
        
        agent.update() # Update at end of episode
        log_data.append([ep+1, score, cov*100, steps])
        
        # [수정] 매 에피소드 출력
        print(f"PPO Ep {ep+1}/{TOTAL_EPISODES} | Ret: {score:.2f} | Cov: {cov*100:.2f}% | Steps: {steps}")
        
    agent.save("ppo_model.pth")
    with open("ppo_training_log.csv", 'w', newline='') as f:
        csv.writer(f).writerow(['Episode', 'Return', 'Coverage', 'Steps'])
        csv.writer(f).writerows(log_data)

def train_rppo():
    print("\n>>> [2/4] Starting R-PPO Training...")
    env = MazeEnv(render_mode=RENDER_MODE, max_steps=MAX_STEPS)
    ...
    for ep in range(TOTAL_EPISODES):
        ...
        while not done:
            ...
            next_state, reward, done, cov, _ = env.step(action)
            agent.buffer['r'].append(reward)
            agent.buffer['d'].append(done)
            state = next_state
            score += reward
            steps += 1
            timestep += 1
            if timestep % 2048 == 0: agent.update()
            
        log_data.append([ep+1, score, cov*100, steps])
        # [수정] 매 에피소드 출력
        print(f"R-PPO Ep {ep+1}/{TOTAL_EPISODES} | Ret: {score:.2f} | Cov: {cov*100:.2f}% | Steps: {steps}")
        
    agent.save("rppo_model.pth")
    with open("rppo_training_log.csv", 'w', newline='') as f:
        csv.writer(f).writerow(['Episode', 'Return', 'Coverage', 'Steps'])
        csv.writer(f).writerows(log_data)

def train_hppo():
    print("\n>>> [3/4] Starting History-PPO Training...")
    env = MazeEnv(render_mode=RENDER_MODE, max_steps=MAX_STEPS)
    stack_frames = 3
    ...
    for ep in range(TOTAL_EPISODES):
        ...
        log_data.append([ep+1, score, cov*100, steps])
        # [수정] 매 에피소드 출력
        print(f"H-PPO Ep {ep+1}/{TOTAL_EPISODES} | Ret: {score:.2f} | Cov: {cov*100:.2f}% | Steps: {steps}")
        
    agent.save("hppo_model.pth")
    with open("hppo_training_log.csv", 'w', newline='') as f:
        csv.writer(f).writerow(['Episode', 'Return', 'Coverage', 'Steps'])
        csv.writer(f).writerows(log_data)

def train_cppo():
    print("\n>>> [4/4] Starting CPPO (Optimized) Training...")
    env = MazeEnv(render_mode=RENDER_MODE, max_steps=MAX_STEPS)
    ...
    for ep in range(TOTAL_EPISODES):
        state = env.reset()
        queue = deque([state] * 4, maxlen=4) # CPPO uses 4 frames
        stacked_state = np.concatenate(list(queue))
        score = 0
        done = False
        steps = 0
        while not done:
            ...
            steps += 1
            timestep += 1
            if timestep % 4096 == 0: agent.update() # CPPO uses larger buffer
        
        log_data.append([ep+1, score, cov*100, steps])
        # [수정] 매 에피소드 출력
        print(f"CPPO Ep {ep+1}/{TOTAL_EPISODES} | Ret: {score:.2f} | Cov: {cov*100:.2f}% | Steps: {steps}")
        
    agent.save("cppo_model.pth")
    with open("cppo_training_log.csv", 'w', newline='') as f:
        csv.writer(f).writerow(['Episode', 'Return', 'Coverage', 'Steps'])
        csv.writer(f).writerows(log_data)
...
if __name__ == "__main__":
    ...
    print(f"\n>>> All training finished in {hours:.2f} hours.")
