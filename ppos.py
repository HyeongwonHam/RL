...
# ==============================================================================
# 0. Global Configuration & Arguments
# ==============================================================================
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

def parse_args():
    parser = argparse.ArgumentParser(description="RL Maze Training/Testing")
    parser.add_argument("--algo", type=str, default="ppo", choices=["ppo", "rppo", "hppo", "cppo"], help="Algorithm to use")
    parser.add_argument("--openness", type=float, default=0.6, help="Maze openness (0.0 ~ 1.0)")
    parser.add_argument("--mode", type=str, default="train", choices=["train", "test"], help="Execution mode")
    parser.add_argument("--episodes", type=int, default=5000, help="Total episodes for training")
...
MODE = args.mode
TOTAL_EPISODES = args.episodes if MODE == 'train' else 10
RENDER_MODE = args.render
MAX_STEPS = 400

MODEL_NAME = f"{ALGO}_open{OPENNESS}_model.pth"
LOG_NAME = f"{ALGO}_open{OPENNESS}_{MODE}_log.csv"
...
# ==============================================================================
# 1. Unified Maze Environment (Your Original Env)
# ==============================================================================
class MazeEnv:
...
# ==============================================================================
# 5. CPPO Agent Class (Updated: 1D CNN with Team's Logic)
# ==============================================================================
class CPPOAgent:
    class CNNActorCritic(nn.Module):
        def __init__(self, input_channels, input_len, action_dim, hidden_size=512):
            super().__init__()
            # [Reflecting Team's Main.py Structure]
            # Conv(64) -> Conv(128) -> Conv(128) with Stride 2
            ...
            # [Fix]: Define Parameter correctly on device to avoid ValueError
    ...
    def __init__(self, state_dim, action_dim):
        ...
        # In this env: state_dim = 38 (36 Rays + 2 Action)
        ...
        # [Fix]: action_var is already on device inside class, just pass parameters
        self.optimizer = optim.Adam(self.policy.parameters(), lr=self.lr)
        
        self.buffer = {'s':[], 'a':[], 'lp':[], 'r':[], 'd':[], 'v':[]}

    def select_action(self, state):
        # Flattened state input (152,) -> Reshape to (4, 38) for CNN
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            for start in range(0, dataset_size, self.batch_size):
                ...
                # [Fix]: Safe Covariance Expansion (using diag_embed)
                # action_var: (2,) -> expand to (Batch, 2)
...
def run_ppo():
    ...
    if MODE == 'test':
        if os.path.exists(MODEL_NAME): agent.load(MODEL_NAME)
        else: print(f"Error: Model {MODEL_NAME} not found."); return
    ...
    for ep in range(TOTAL_EPISODES):
        ...
        while not done:
            ...
            if MODE == 'train':
                ...
                agent.buffer['d'].append(done)
                timestep += 1
                if timestep % 4096 == 0: agent.update()
