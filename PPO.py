...
MODEL_PATH = "ppo_mapping_model.pth"
LOG_FILE = "ppo_training_log.csv" # [추가] 로그 파일 경로
...
class PPOAgent:
    def __init__(self, state_dim, action_dim, p):
        ...
        self.policy = ActorCritic(state_dim, action_dim, p['hidden_size']).to(device)
        self.optimizer = optim.Adam([
            {'params': self.policy.actor.parameters(), 'lr': p['lr_actor']},
            {'params': self.policy.critic.parameters(), 'lr': p['lr_critic']}
        ])
        
        self.policy_old = ActorCritic(state_dim, action_dim, p['hidden_size']).to(device)
...
class MazeEnv:
    def __init__(self, maze_width=8, maze_height=8, cell_size=1.0, render_mode=False, max_steps=300):
        ...
        if self.render_mode:
            ...
            self.pygame.display.set_caption("PPO Comparison Agent")
...
if __name__ == "__main__":
    ...
    ppo_agent = PPOAgent(state_dim, action_dim, params)
    ...
    for ep in range(episodes):
        ...
        if EXEC_MODE == 'train':
            # [수정] 리스트에 데이터 저장 (파일 쓰기 X)
            log_data.append([ep+1, ep_reward, cov*100, ep_steps])

            if (ep+1) % 50 == 0:
                ppo_agent.save_checkpoint(MODEL_PATH)
            
    # [추가] 모든 에피소드 종료 후 한 번에 저장
    if EXEC_MODE == 'train':
